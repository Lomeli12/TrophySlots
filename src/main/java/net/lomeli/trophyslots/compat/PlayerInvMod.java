        ...
        if (slotList != null) {
            for (int i = 5; i < slotList.size(); i++) {
                ...
                if (slot != null && !(slot instanceof SlotCrafting)) {
                    if (slot.inventory != containerPlayer.craftMatrix && slot.isSlotInInventory(player.inventory, slot.getSlotIndex()) && !TrophySlots.proxy.slotUnlocked(slot.getSlotIndex()))
                        containerPlayer.inventorySlots.set(i, SlotLocked.getSlot(player, slot));
                }
            }
        }
